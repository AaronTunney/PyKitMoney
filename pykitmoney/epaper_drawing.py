"""Raspberry Pi e-paper display drawing."""

import logging
from PIL import Image, ImageDraw
import PIL
from pykitmoney import utils, draw_utils
...
def draw_to_epaper_display(name, balance, transactions, next_transfer):
    ...
    Args:
        ...
        balance: The balance string.
        transactions: List of transaction strings.
        next_transfer: Next transfer information string.
    """
    ...
    try:
        ...
        title_font = draw_utils.largest_font_for(
            title, title_font, title_width - (2 * margin), title_height - (2 * margin)
        )
        (title_x, title_y) = draw_utils.origin_to_center_text(
            title, title_font, title_width, title_height
        )
        ...
        balance_font = draw_utils.largest_font_for(
            balance, balance_font, balance_width - (2 * margin), balance_height - (2 * margin)
        )
        (balance_x, balance_y) = draw_utils.origin_to_center_text(
            balance, balance_font, balance_width, balance_height
        )
        draw.text(
            (balance_x, balance_y_origin + balance_y - margin), balance, font=balance_font, fill=0
        )
        ...
        next_font = draw_utils.largest_font_for(
            next_title, next_font, next_width - (2 * margin), title_height - (2 * margin)
        )
        (next_x, next_y) = draw_utils.origin_to_center_text(
            next_title, next_font, next_width, title_height
        )
        ...
        if len(transactions) > 0:
            ...
            (transaction_x, transaction_y) = draw_utils.origin_to_center_text(
                longest_text, transactions_font, transactions_width, transactions_height
            )
